...
TEST(CameraNodeTest, CameraNodeCanBePluggedToScreenNode)
{
	...
	EXPECT_TRUE(
	    Math::eq(cameraNode->getProj()->getData(I3T_SEQ_OUT_MAT).getMat4(),
	             perspectiveProj->getData().getMat4()));
	EXPECT_TRUE(
	    Math::eq(cameraNode->getView()->getData(I3T_SEQ_OUT_MAT).getMat4(),
	             lookAt->getData().getMat4()));

	plug_expectOk(cameraNode, screenNode, 0, 0);

	auto expectedPV =
	    perspectiveProj->getData().getMat4() * lookAt->getData().getMat4();
...
TEST(CameraNodeTest, CameraAndSequenceCannotBeConnected)
{
	...
	const auto result =
	    GraphManager::plug(camera, sequence, I3T_CAMERA_OUT_MUL, I3T_SEQ_IN_MUL);
