...
template <typename T>
inline void setValue_expectOk(NodePtr node, T&& value, glm::ivec2 coords)
{
...
inline void plug_expectOk(T1&& lhs, T2&& rhs, int leftIndex = 0,
                          int rightIndex = 0)
{
	auto plugResult = GraphManager::plug(
	    std::forward<T1>(lhs), std::forward<T2>(rhs), leftIndex, rightIndex);
