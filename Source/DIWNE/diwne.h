...
typedef std::function<void(...)>
    popupContent_function_pointer; /*!< \brief you can pass any arguments to you function with popup menu content */
...
struct SettingsDiwne
{
	...
	std::string const editorlabel =
	    "diwneBackground"; /*!< as well as all other DiwneObject, Diwne has to have identification label too */
	ImRect const workAreaDiwne = ImRect(
	    0, 0, 0,
	    0); /*!< workarea in Diwne coordinates (so what part of infinite space of node editor is on window) only initial value - mostly based on window size ( \see updateWorkAreaRectangles() ) */
	...
	ImColor selectionRectFullColor =
	    ImColor(0, 0, 255, 100); /* \todo JH recomandation is not to store ImColor, but ImU32 or ImVec4 */
...
public:
	...
	bool
	blockRaisePopup(); /*!< sometimes we do not want to raise popup - here specify it ( now it is when selecting action run ) */
	...
	/*! \brief Draw filled rectangle to window ImDrawlist
         ...
         *
         */
	void AddRectFilledDiwne(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f,
	...
	/*! \brief Draw rectangle to window ImDrawlist \see AddRectFilledDiwne
         */
	void AddRectDiwne(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f,
	...
	/*! \brief Draw Bezier (not Bezier really) curve to window ImDrawList
         ...
         *
         */
	void AddBezierCurveDiwne(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col,
	...
	bool m_popupDrawn /*!< not draw popup two times \todo maybe unused when every object is drawn just one time */,
	    m_tooltipDrawn /*!< not draw tooltip two times \todo maybe unused when every object is drawn just one time */,
	    m_objectFocused /*!< only one object can be focused */,
	    m_allowUnselectingNodes /*!< for example when holding ctrl nodes not going unselected when sleection rect get out of them */
	    ;
...
private:
	...
	ImRect
	      m_workAreaDiwne; /*! \brief Rectangle of work area on diwne - .Min is set by user, .Max is computed from m_workAreaScreen */
