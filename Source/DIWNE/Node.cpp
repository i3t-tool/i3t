...
Node::Node(DIWNE::Diwne& diwne, DIWNE::ID id,
           std::string const labelDiwne /*="DiwneNode"*/)
    : DiwneObject(diwne, id, labelDiwne), m_topRectDiwne(ImRect(0, 0, 0, 0)),
      ...
      m_drawAnywhere(true), m_nodePosMode(OnItsPosition), m_toDelete(false)
{
}
...
void Node::end()
{
	...
	DIWNE_DEBUG((diwne), {
		...
		ImGui::Text(fmt::format("D:{}-{}-{}-{}\nWA:{}-{}-{}-{}\nS:{}-{}-{}-{}",
		                        nodeRectDiwne.Min.x, nodeRectDiwne.Min.y,
		                        nodeRectDiwne.Max.x, nodeRectDiwne.Max.y,
		                        diwne.diwne2workArea(nodeRectDiwne.Min).x,
		                        diwne.diwne2workArea(nodeRectDiwne.Min).y,
		                        diwne.diwne2workArea(nodeRectDiwne.Max).x,
		                        diwne.diwne2workArea(nodeRectDiwne.Max).y,
		                        diwne.diwne2screen(nodeRectDiwne.Min).x,
		                        diwne.diwne2screen(nodeRectDiwne.Min).y,
		                        diwne.diwne2screen(nodeRectDiwne.Max).x,
		                        diwne.diwne2screen(nodeRectDiwne.Max).y)
		                .c_str());
	});                /* close of macro */
#endif               // DIWNE_DEBUG
...
bool Node::content()
{
	...
	{
		ImGui::Indent((m_leftRectDiwne.GetWidth() +
		               m_centerDummySpace * diwne.mp_settingsDiwne->middleAlign) *
	...
	{
		ImGui::Indent(
		    (m_middleRectDiwne.GetWidth() +
		     m_centerDummySpace * (1 - diwne.mp_settingsDiwne->middleAlign)) *
		        diwne.getWorkAreaZoom() +
		    ImGui::GetStyle().ItemSpacing.x);
	}
...
bool Node::afterEndDiwne()
{
	if (m_selectable)
	{
		...
		if (diwne.getDiwneActionPreviousFrame() ==
		        DIWNE::DiwneAction::SelectionRectFull ||
		...
		if (m_selected)
		{
			diwne.AddRectDiwne(
			    ...
			    diwne.mp_settingsDiwne->itemSelectedBorderColor, 0,
	...
	ImGui::InvisibleButton("IBBlockingOtherImGuiInteractions",
	                       getNodeRectSizeDiwne() * diwne.getWorkAreaZoom());
...
{
	translateNodePositionDiwne(diwne.bypassGetMouseDelta() /
	                           diwne.getWorkAreaZoom());
	return true;
}
...
#ifdef DIWNE_DEBUG
	DIWNE_DEBUG((diwne), {
		diwne.AddRectDiwne(m_topRectDiwne.Min, m_topRectDiwne.Max,
		                   ImColor(0, 0, 255, 100), 0, ImDrawCornerFlags_None, 2);
	});  /* close of macro */
#endif // DIWNE_DEBUG
...
#ifdef DIWNE_DEBUG
	DIWNE_DEBUG((diwne), {
		diwne.AddRectDiwne(m_leftRectDiwne.Min, m_leftRectDiwne.Max,
		                   ImColor(0, 0, 255, 100), 0, ImDrawCornerFlags_None, 2);
	});  /* close of macro */
#endif // DIWNE_DEBUG
...
#ifdef DIWNE_DEBUG
	DIWNE_DEBUG((diwne), {
		diwne.AddRectDiwne(m_middleRectDiwne.Min, m_middleRectDiwne.Max,
		                   ImColor(0, 0, 255, 100), 0, ImDrawCornerFlags_None, 2);
	});  /* close of macro */
#endif // DIWNE_DEBUG
...
#ifdef DIWNE_DEBUG
	DIWNE_DEBUG((diwne), {
		diwne.AddRectDiwne(m_rightRectDiwne.Min, m_rightRectDiwne.Max,
		                   ImColor(0, 0, 255, 100), 0, ImDrawCornerFlags_None, 2);
	});  /* close of macro */
#endif // DIWNE_DEBUG
...
#ifdef DIWNE_DEBUG
	DIWNE_DEBUG((diwne), {
		diwne.AddRectDiwne(m_bottomRectDiwne.Min, m_bottomRectDiwne.Max,
		                   ImColor(0, 0, 255, 100), 0, ImDrawCornerFlags_None, 2);
	});  /* close of macro */
#endif // DIWNE_DEBUG
...
void Node::updateSizes()
{
	...
	ImVec2 spacing =
	    ...
	    diwne.getWorkAreaZoom(); /* in BeginDiwne() is ItemSpacing scaled */

	float rightWidth = m_rightRectDiwne.GetWidth();
	float centerWidth =
	    m_leftRectDiwne.GetWidth() + m_middleRectDiwne.GetWidth() + rightWidth +
	    spacing.x * 2; /* space is between left-middle and middle-right */
	float maxWidth =
	    std::max(m_topRectDiwne.GetWidth(),
	             std::max(centerWidth, m_bottomRectDiwne.GetWidth()));

	float bottomYOfCentre = m_bottomRectDiwne.Min.y - spacing.y;

	m_topRectDiwne.Max.x =
	    m_topRectDiwne.Min.x + maxWidth; /* top.Max.x is most right point now */
