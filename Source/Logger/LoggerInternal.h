...
#ifdef _DEBUG
#define LOG_DEBUG(...)                                                         \
	Logger::getInstance().getConsoleLogger()->trace(__VA_ARGS__);
#else
#define LOG_DEBUG(...)
#endif
#define LOG_INFO(...)                                                          \
	Logger::getInstance().getConsoleLogger()->info(__VA_ARGS__);
#define LOG_WARN(...)                                                          \
	Logger::getInstance().getConsoleLogger()->warn(__VA_ARGS__);
#define LOG_ERROR(...)                                                         \
	Logger::getInstance().getConsoleLogger()->error(__VA_ARGS__);
#define LOG_FATAL(...)                                                         \
	Logger::getInstance().getConsoleLogger()->critical(__VA_ARGS__);
#define LOG_EVENT_MOUSE_POS(mouseX, mouseY)                                    \
	Logger::getInstance().log(LoggingOption::MOUSE_MOVEMENT,                     \
	                          Logger::getInstance().getLogString("mousePos"), 2, \
	                          mouseX, mouseY)
#define LOG_EVENT_MOUSE_CLICK(button, mouseX, mouseY)                          \
	Logger::getInstance().log(LoggingOption::MOUSE_CLICK,                        \
	                          Logger::getInstance().getLogString("mouseClick"),  \
	                          3, button, mouseX, mouseY)
#define LOG_EVENT_MOUSE_RELEASE(button, mouseX, mouseY)                        \
	Logger::getInstance().log(                                                   \
	    LoggingOption::MOUSE_CLICK,                                              \
	    Logger::getInstance().getLogString("mouseRelease"), 3, button, mouseX,   \
	    mouseY)
#define LOG_EVENT_OPEN_POP_UP(clickedTab)                                      \
	Logger::getInstance().log(LoggingOption::POP_UP,                             \
	                          Logger::getInstance().getLogString("openPopUp"),   \
	                          1, clickedTab->getNameableParentNameId())
#define LOG_EVENT_CLOSE_POP_UP(tab)                                            \
	Logger::getInstance().log(LoggingOption::POP_UP,                             \
	                          Logger::getInstance().getLogString("closePopUp"),  \
	                          1, tab->getNameableParentNameId())
#define LOG_EVENT_CLOSE_POP_UP_IN(tab)                                         \
	Logger::getInstance().log(                                                   \
	    LoggingOption::POP_UP,                                                   \
	    Logger::getInstance().getLogString("closePopUpIn"), 1,                   \
	    tab->getNameableParentNameId())
#define LOG_EVENT_BUTTON_CLICK(tab)                                            \
	if (dynamic_cast<Button*>(tab))                                              \
	Logger::getInstance().log(LoggingOption::POP_UP,                             \
	                          Logger::getInstance().getLogString("button"), 1,   \
	                          tab->getNameableParentNameId())
#define LOG_EVENT_CONNECT(startTab, endTab)                                    \
	Logger::getInstance().log(                                                   \
	    LoggingOption::LOGIC, Logger::getInstance().getLogString("connect"), 2,  \
	    startTab->getNameableParentNameId(), endTab->getNameableParentNameId())
#define LOG_EVENT_DISCONNECT(startTab, endTab)                                 \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("disconnect"),  \
	                          2, startTab->getNameableParentNameId(),            \
	                          endTab->getNameableParentNameId())
#define LOG_EVENT_TAB_ADDED_AT_INDEX(index, addedTab, addedToTab)              \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("tabAddIndex"), \
	                          3, addedTab->getNameableParentNameId(),            \
	                          addedToTab->getNameableParentNameId(), index)
#define LOG_EVENT_TAB_ADDED(addedTab, addedToTab)                              \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("tabAdd"), 2,   \
	                          addedTab->getNameableParentNameId(),               \
	                          addedToTab->getNameableParentNameId())
#define LOG_EVENT_TAB_REMOVED(removedTab, removedFromTab)                      \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("tabRem"), 2,   \
	                          removedTab->getNameableParentNameId(),             \
	                          removedFromTab->getNameableParentNameId())
#define LOG_EVENT_OBJECT_ADDED(objectName, tab)                                \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("objAdd"), 2,   \
	                          objectName, tab->getNameableParentNameId())
#define LOG_EVENT_OBJECT_REMOVED(tab)                                          \
	Logger::getInstance().log(LoggingOption::LOGIC,                              \
	                          Logger::getInstance().getLogString("objRem"), 1,   \
	                          tab->getNameableParentNameId())
#define LOG_EVENT_MATRIX_VALUE_UPDATE(tabGroup, matrix, index, newVal)         \
	if (tabGroup->getDraggedNumberBox() == NULL)                                 \
	Logger::getInstance().log(LoggingOption::MATRIX_FIELD,                       \
	                          Logger::getInstance().getLogString("matrix"), 3,   \
	                          index, matrix->getNameableParentNameId(), newVal)
#define LOG_EVENT_MATRIX_VALUE_UPDATE_DRAG(matrix, index, newVal)              \
	Logger::getInstance().log(LoggingOption::MATRIX_FIELD,                       \
	                          Logger::getInstance().getLogString("matrix"), 3,   \
	                          index, matrix->getNameableParentNameId(), newVal)
#define LOG_EVENT_TUTORIAL_STEP(tutorial, step, name)                          \
	Logger::getInstance().log(LoggingOption::TUTORIAL,                           \
	                          Logger::getInstance().getLogString("tutorial"), 3, \
	                          tutorial, step, name)
...
public:
	...
	std::shared_ptr<spdlog::logger> getAppLogger() { return appLogger; };
	...
	void update();
	void log(const LoggingOption& logType, const std::string message,
	         const int numOfArgs, ...);
	void addToLogBuffer(const LoggingOption& logType, const std::string message,
	                    const int numOfArgs, ...);
...
private:
	...
	// spdlog::basic_logger_mt("app_logger", "logs/main.log");
	std::shared_ptr<spdlog::logger> appLogger;
	std::shared_ptr<spdlog::logger> logger =
	    spdlog::basic_logger_mt("basic_logger", "logs/user_interaction.log");
	std::shared_ptr<spdlog::logger> mouseLogger =
	    spdlog::basic_logger_mt("mouse_logger", "logs/mouse_log.log");
	...
	void vAddToLogBuffer(const LoggingOption& logType, const std::string& message,
	                     const int numOfArgs, va_list& arguments);
	...
	const std::string LOG_STRINGS_PATH =
	    Config::getAbsolutePath("Data/events_definition.json");
