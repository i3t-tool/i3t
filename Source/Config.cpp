...
bool Config::getMat4(std::istream& is, const std::string& input, const std::string& attribName, glm::mat4& val)
{
  if (input == attribName)
  {
    is >> val[0][0] >> val[1][0] >> val[2][0] >> val[3][0] >> val[0][1] >> val[1][1] >> val[2][1] >> val[3][1] >> val[0]
      [2] >> val[1][2] >> val[2][2] >> val[3][2] >> val[0][3] >> val[1][3] >> val[2][3] >> val[3][3];
    if (is.good())
    {
      //cout << key << " : " << std::endl;
...
void Config::loadFromFile(const std::string filename)
{
  ...
  while (!is.eof() && is.good())
  {
    ...
    if (getBool(is, input, "SHOW_CONSOLE", b)) SHOW_CONSOLE = b;
    if (getBool(is, input, "FULLSCREEN", b)) FULLSCREEN = b;

    if (getVec3(is, input, "BACKGROUND_COLOR", vec)) BACKGROUND_COLOR = vec;

    if (getValue(is, input, "WIN_HEIGHT", val)) WIN_HEIGHT = (int)val;
    if (getValue(is, input, "WIN_WIDTH", val)) WIN_WIDTH = (int)val;

    if (getValue(is, input, "CAM_NEAR", val)) CAM_NEAR = val;
    if (getValue(is, input, "CAM_FAR", val)) CAM_FAR = val;
    if (getValue(is, input, "CAM_MOTION_SENSITIVITY", val)) CAM_MOTION_SENSITIVITY = val;
    if (getValue(is, input, "CAM_SCROLL_SENSITIVITY", val)) CAM_SCROLL_SENSITIVITY = val;
    if (getValue(is, input, "MOUSE_SENSITIVITY", val)) MOUSE_SENSITIVITY = val;


    // content
    if (getWord(is, input, "CONTENT_FILE", word)) CONTENT_FILE = word;
    /*
    if (getWord(is, input, "GUI_CFG", word))
      TabsConfig::readFromFile(getAbsolutePath(word.c_str()).c_str());
     */
    if (getWord(is, input, "LOAD_SCENE", word)) LOAD_SCENE = word;
...
std::string Config::INFO_TEXT = "Learn geometric transformations.\n\n"
  ...
  "This tool is a result of master's thesis done at the Department of Computer Graphics and Interaction, FEL, CTU in Prague";
