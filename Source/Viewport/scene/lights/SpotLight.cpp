...
void SpotLight::setUniforms(const PhongShader& shader, int index) const
{
	...
	glUniform3fv(glGetUniformLocation(shader.id, (prefix + ".position").c_str()), 1, glm::value_ptr(pos));
	...
	glUniform1f(glGetUniformLocation(shader.id, (prefix + ".cutoffSoftAngle").c_str()),
