...
SceneModel::SceneModel(Core::Mesh* mesh, PhongShader* shader)
    : TexturedObject(mesh, shader)
{
	// Empty
}
...
void SceneModel::update(Scene& scene)
{
	...
	axesPtr->m_modelMatrix =
	    this->m_modelMatrix * glm::scale(glm::mat4(1.0f), glm::vec3(2.0f));
}

void SceneModel::onSceneAdd(Scene& scene)
{
	TexturedObject::onSceneAdd(scene);
	auto axes = std::make_shared<GameObject>(
	    RMI.meshByAlias(Shaper::xyzAxes), scene.m_viewport->m_colorShader.get());
