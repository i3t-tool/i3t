...
//  2020-XX-XX: Platform: Added support for multiple windows via the
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
	{
		...
		if (focused)
		{
			...
			else
			{
				...
				if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
				{
					...
					io.MousePos =
					    ImVec2((float)mouse_x + window_x, (float)mouse_y + window_y);
				}
				else
				{
		...
		// (Optional) When using multiple viewports: set io.MouseHoveredViewport to
		// the viewport the OS mouse cursor is hovering. Important: this information
		// is not easy to provide and many high-level windowing library won't be
		// able to provide it correctly, because
		// - This is _ignoring_ viewports with the ImGuiViewportFlags_NoInputs flag
		// (pass-through windows).
		// - This is _regardless_ of whether another viewport is focused or being
		// dragged from. If ImGuiBackendFlags_HasMouseHoveredViewport is not set by
		// the back-end, imgui will ignore this field and infer the information by
		// relying on the rectangles and last focused time of every viewports it
		// knows about. It will be unaware of other windows that may be sitting
		// between or over your windows. [GLFW] FIXME: This is currently only
		// correct on Win32. See what we do below with the WM_NCHITTEST, missing an
		// equivalent for other systems. See
...
{
	ImGuiIO& io = ImGui::GetIO();
	IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally "
	                                 "built by the renderer back-end. Missing "
	                                 "call to renderer _NewFrame() function? "
...
static void ImGui_ImplGlfw_WindowPosCallback(GLFWwindow* window, int, int)
{
	if (ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle(window))
	{
		...
		{
			bool ignore_event =
			    (ImGui::GetFrameCount() <= data->IgnoreWindowPosEventFrame + 1);
...
static void ImGui_ImplGlfw_WindowSizeCallback(GLFWwindow* window, int, int)
{
	if (ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle(window))
	{
		...
		{
			bool ignore_event =
			    (ImGui::GetFrameCount() <= data->IgnoreWindowSizeEventFrame + 1);
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
	...
	data->Window = glfwCreateWindow((int)viewport->Size.x, (int)viewport->Size.y,
...
static void ImGui_ImplGlfw_SetWindowFocus(ImGuiViewport* viewport)
{
	...
	// doesn't actually call SetWindowFocus
                                          ...
                                          ImU64* out_vk_surface)
{
	...
	VkResult err =
	    glfwCreateWindowSurface((VkInstance)vk_instance, data->Window,
