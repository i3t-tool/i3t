...
// (Info: GLFW is a cross-platform general purpose library for handling windows, inputs, OpenGL/Vulkan graphics
// context creation, etc.) (Requires: GLFW 3.1+. Prefer GLFW 3.3+ for full feature support.)

// Implemented features:
//  [X] Platform: Clipboard support.
//  [X] Platform: Gamepad support. Enable with 'io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad'.
//  [X] Platform: Mouse cursor shape and visibility. Disable with 'io.ConfigFlags |=
//  ImGuiConfigFlags_NoMouseCursorChange' (note: the resizing cursors requires GLFW 3.4+). [X] Platform: Keyboard
//  arrays indexed using GLFW_KEY_* codes, e.g. ImGui::IsKeyPressed(GLFW_KEY_SPACE). [X] Platform: Multi-viewport
//  support (multiple windows). Enable with 'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'.

// Issues:
//  [ ] Platform: Multi-viewport support: ParentViewportID not honored, and so io.ConfigViewportsNoDefaultParent has
//  no effect (minor).
...
//  2020-01-17: Inputs: Disable error callback while assigning mouse cursors because some X11 setup don't have them
//  and it generates errors. 2019-12-05: Inputs: Added support for new mouse cursors added in GLFW 3.4+ (resizing
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
	{
		...
		// If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the back-end, imgui will ignore this field and infer
		// the information by relying on the rectangles and last focused time of every viewports it knows about. It will
		// be unaware of other windows that may be sitting between or over your windows. [GLFW] FIXME: This is currently
		// only correct on Win32. See what we do below with the WM_NCHITTEST, missing an equivalent for other systems. See
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
	{
		...
		else
		{
			...
			glfwSetCursor(
			    window, g_MouseCursors[imgui_cursor] ? g_MouseCursors[imgui_cursor] : g_MouseCursors[ImGuiMouseCursor_Arrow]);
...
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCHITTEST)
	{
		// Let mouse pass-through the window. This will allow the back-end to set io.MouseHoveredViewport properly (which
                                          ...
                                          ImU64* out_vk_surface)
{
	...
	VkResult err = glfwCreateWindowSurface((VkInstance) vk_instance, data->Window,
