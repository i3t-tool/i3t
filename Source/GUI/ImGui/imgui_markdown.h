#pragma once
//#include <imgui.h> // temporary only because of error highlighting in VS
...
#include "IconsFontAwesome5.h"  // https://github.com/juliettef/IconFontCppHeaders // todo check out
...
inline void defaultMarkdownTooltipCallback(MarkdownTooltipCallbackData data_)
{
	...
	{
		if (data_.linkData.textLength > 0) { ImGui::SetTooltip("%.*s", data_.linkData.textLength, data_.linkData.text); }
	}
	else
	{
...
struct TextRegion
{
	TextRegion() : indentX(0.0f), listCounter(1) {}
	...
	// CUSTOM CODE
	void RenderOrderedListTextWrapped(const char* text_, const char* text_end_)
	{
		std::string numStr = std::to_string(listCounter) + ")";
		listCounter++;
...
inline void RenderLine(const char* markdown_, Line& line_, TextRegion& textRegion_, const MarkdownConfig& mdConfig_)
{
	...
	// CUSTOM CODE
	else if (line_.isOrderedListStart) // render unordered list
	{
		formatInfo.type = MarkdownFormatType::ORDERED_LIST;
		mdConfig_.formatCallback(formatInfo, true);
		const char* text = markdown_ + textStart + 1;
		textRegion_.RenderOrderedListTextWrapped(text, text + textSize - 1);
	}
...
inline void Markdown(const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_)
{
	...
	for (int i = 0; i < (int) markdownLength_; ++i)
	{
		...
		if (line.isLeadingSpace)
		{
			...
			else
			{
				...
				// CUSTOM CODE
				else if (c == '+') // && ( line.leadSpaceCount >= 2 )
				{
					if (((int) markdownLength_ > i + 1) && (markdown_[i + 1] == ' ')) // space after '*'
					{
						line.isOrderedListStart = true;
						++i;
						++line.lastRenderPosition;
					}
					// carry on processing as could be emphasis
		...
		case Link::HAS_SQUARE_BRACKETS_ROUND_BRACKET_OPEN:
			...
			if (link.num_brackets_open == 0)
			{
				...
				if (link.isImage)                  // it's an image, render it.
				{
					...
					if (mdConfig_.imageCallback)
					{
						...
						if (imageData.isValid)
						{
							...
							//	ImGui::NewLine();
							//}
							ImGui::Image(imageData.user_texture_id, imageData.size, imageData.uv0, imageData.uv1, imageData.tint_col,
				...
				{
					textRegion.RenderLinkTextWrapped(markdown_ + link.text.start, markdown_ + link.text.start + link.text.size(),
					                                 link, markdown_, mdConfig_, &linkHoverStart, false);
				}
	...
	if (em.state == Emphasis::LEFT && line.emphasisCount >= 3) { ImGui::Separator(); }
	else
	{
...
inline void defaultMarkdownFormatCallback(const MarkdownFormatInfo& markdownFormatInfo_, bool start_)
{
	...
	case MarkdownFormatType::LINK:
		if (start_) { ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_ButtonHovered]); }
		else
		{
			ImGui::PopStyleColor();
			if (markdownFormatInfo_.itemHovered) { ImGui::UnderLine(ImGui::GetStyle().Colors[ImGuiCol_ButtonHovered]); }
			else
			{
