...
private:
	...
	GameObject* m_sceneModel =
	    nullptr; // model in the 3D scene (in the hierarchical scene graph)
	GameObject* m_workspaceModel =
	    nullptr; // model in the workspace (as a preview in the box)

	// variables of the workspace box
	GLuint m_textureID = 0; // rendered texture name (COLOR_ATTACHMENT0 in m_fbo)
	ImVec2 m_textureSize = {84, 100}; // initial render texture size - should be
	...
	GLuint m_fbo = 0; // FBO for texture rendering - needed for additional
	                  // rendering to the m_textureID only
	RenderTexture* renderTexture; // needed when creating camera only, can be a
	                              // local variable
