...
int WorkspaceSequence::getInnerPosition(std::vector<ImVec2> points)
{
	...
	for (auto const& innerNode : getInnerWorkspaceNodes())
	{
		...
		if (rect.Contains(in_point)) { return i; }
...
bool WorkspaceSequence::middleContent()
{
	...
	if (m_levelOfDetail == WorkspaceLevelOfDetail::Label) { return false; }
	...
	if (getInputs().at(Core::I3T_SEQ_IN_MAT)->isConnected())
	{
		...
		return drawData4x4(
		    ...
		    m_nodebase->getData(0).getMat4() /*\todo JM HM better selection (index) of data*/,
	    ...
	    diwne.getDiwneActionPreviousFrame() == DIWNE::DiwneAction::DragNode)
	{
		...
		if (dragedNode != nullptr &&
		    (dragedNode->aboveSequence == 0 ||
		     dragedNode->aboveSequence ==
		         getIdDiwne())) /* only transformation can be in Sequence && not above other sequence */
		{
	...
	m_workspaceInnerTransformations.erase(
	    std::remove_if(m_workspaceInnerTransformations.begin(), m_workspaceInnerTransformations.end(),
	                   [](Ptr<WorkspaceNodeWithCoreData> const& node) -> bool {
		                   return std::dynamic_pointer_cast<WorkspaceTransformation>(node)->getRemoveFromSequence();
	                   }),
	...
	for (auto const& transformation : m_workspaceInnerTransformations)
	{
		if (position_of_draged_node_in_sequence == i)
		{
			...
			ImGui::SameLine();
			if (dragedNode->bypassUnholdAction()) { push_index = i; }
		}
		...
		inner_interaction_happen |= transformation->drawNodeDiwne<WorkspaceTransformation>(
	...
	{
		ImGui::Dummy(position_of_draged_node_in_sequence == i ? m_sizeOfDummy * diwne.getWorkAreaZoom()
		                                                      : m_sizeOfDummy * diwne.getWorkAreaZoom() /
		                     2); /* smaller dummy if dragged node is not over Sequence */
		if (dragedNode && dragedNode->bypassUnholdAction() && position_of_draged_node_in_sequence >= 0) { push_index = i; }
	}
		...
		if (dragedNode != nullptr) /* only transformation can be in Sequence*/
		{
			if (dragedNode->isInSequence() && dragedNode->getNodebaseSequence() == m_nodebase)
			{
				...
				inner_interaction_happen =
				    false; /* do not set sequence as interacting node -> draged transformation should be only interacting */
			}
		}
	}
