...
void WorkspaceSequence::popNode(Ptr<WorkspaceNodeWithCoreData> node)
{
	...
	auto node_iter =
	    std::find_if(m_workspaceInnerTransformations.begin(),
	                 m_workspaceInnerTransformations.end(),
	                 [node](Ptr<WorkspaceNodeWithCoreData> const& in_node) -> bool
	                 { return node == in_node; });
...
void WorkspaceSequence::pushNode(Ptr<WorkspaceNodeWithCoreData> node, int index)
{
	...
	if (node_t != nullptr && 0 <= index &&
	    index <= m_workspaceInnerTransformations.size())
	{
...
void WorkspaceSequence::moveNodeToSequence(
    Ptr<WorkspaceNodeWithCoreData> dragedNode, int index)
{
...
WorkspaceSequence::getInnerWorkspaceNodes() const
{
...
bool WorkspaceSequence::middleContent()
{
		...
		if (dragedNode != nullptr &&
		    (dragedNode->aboveSequence == 0 ||
		     dragedNode->aboveSequence ==
		         getIdDiwne())) /* only transformation can be in Sequence && not
		                           above other sequence */
		{
			position_of_draged_node_in_sequence =
			    getInnerPosition(dragedNode->getInteractionPointsWithSequence());
			if (position_of_draged_node_in_sequence >= 0)
			{
				dragedNode->aboveSequence =
				    getIdDiwne(); /* reset in transformation beforeBegin */
			}
#ifdef WORKSPACE_DEBUG
			ImGui::Text(fmt::format("Draged node in Sequence: {}",
			                        position_of_draged_node_in_sequence)
			                .c_str());
	...
	{
		moveNodeToSequence(
		    std::dynamic_pointer_cast<WorkspaceNodeWithCoreData>(dragedNode),
		    push_index);
	...
	if (m_drawMode == DIWNE::DrawMode::Interacting &&
	    diwne.getDiwneAction() == DIWNE::DiwneAction::DragNode)
	{
		dragedNode = diwne.getLastActiveNode<WorkspaceTransformation>();
		if (dragedNode != nullptr) /* only transformation can be in Sequence*/
		{
			if (dragedNode->isInSequence() &&
			    dragedNode->getNodebaseSequence() == m_nodebase)
			{
				moveNodeToWorkspace(dragedNode);
				inner_interaction_happen =
				    false; /* do not set sequence as interacting node -> draged
