...
WorkspaceSequence::WorkspaceSequence(ImTextureID headerBackground, Ptr<Core::Sequence> nodebase,
																		 std::string headerLabel, std::string nodeLabel) :
		WorkspaceNodeWithCoreData(headerBackground, nodebase == nullptr ? Core::Builder::createSequence() : nodebase,
...
void WorkspaceSequence::popNode(Ptr<WorkspaceNodeWithCoreData> node)
{
	...
	auto node_iter = std::find_if(
			m_workspaceTransformation.begin(), m_workspaceTransformation.end(),
...
{
	return m_workspaceTransformation;
}
...
void WorkspaceSequence::drawNode(util::NodeBuilder& builder, Core::Pin* newLinkPin, bool withPins)
{
	...
	ImVec2 dataLeftTop = ne::GetNodePosition(m_id) +
			ImVec2(I3T::getSize(ESizeVec2::Nodes_IconSize).x + 5,
						 builder.HeaderMax.y - builder.HeaderMin.y +
								 1); /* \todo JH add shift based on size of header and inputs pins */
	...
	for (auto const& transformation : m_workspaceTransformation)
	{
		...
		if (y > height || i == 0) { m_dataRect.Max.y += y - height; }
...
void WorkspaceSequence::drawDataSetValues(util::NodeBuilder& builder) { drawDataFull(builder, 0); }
...
void WorkspaceSequence::drawDataFull(util::NodeBuilder& builder, int index) { ImGui::Dummy(getDataSize()); }

int WorkspaceSequence::maxLenghtOfData()
{
	Debug::Assert(false,
								"Calling WorkspaceSequence::maxLenghtOfData() make no sense because every included Transformation has "
