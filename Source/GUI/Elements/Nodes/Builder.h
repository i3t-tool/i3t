...
private:
	template <int N, int Max> constexpr void doFor() noexcept
	{
		...
		m_createFns[magic_enum::enum_name(enumValue)] =
		    addNodeToNodeEditorNoSave<WorkspaceOperator<enumValue>>;
...
public:
	...
	{
		I3T_ASSERT(m_createFns.count(type) == 1);
		return std::static_pointer_cast<WorkspaceTransformation>(
		    m_createFns.at(type)(ImVec2(0.0f, 0.0f)));
	}
...
private:
	template <int N, int Max> constexpr void doFor() noexcept
	{
		...
		m_createFns[magic_enum::enum_name(enumValue)] =
		    addNodeToNodeEditorNoSave<WorkspaceTransformation_s<enumValue>>;
