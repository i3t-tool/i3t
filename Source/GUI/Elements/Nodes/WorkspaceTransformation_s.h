...
public:
	...
	void drawMenuLevelOfDetail()
	{
		...
		//                                                              :
		//                                                              {WorkspaceLevelOfDetail::Full,
		//                                                              WorkspaceLevelOfDetail::SetValues,
		//                                                              WorkspaceLevelOfDetail::Label});
	}
	...
	bool drawDataSetValues() override
	{
		...
		if (ImGui::BeginTable(
		        fmt::format("##LoD{}", getIdDiwne()).c_str(), 2 /* label, value */,
			...
			for (auto& [key, valueStore] : nodebase->getDefaultValues())
			{
				...
				case EValueType::Vec3:
				{
					...
					inner_interaction_happen |= drawDataSetValues_InsideTablebuilder(
					    {fmt::format("{} X", key.c_str()),
					     fmt::format("{} Y", key.c_str()),
					     fmt::format("{} Z", key.c_str())}
				...
				case EValueType::Float:
				{
					...
					inner_interaction_happen |= drawDataSetValues_InsideTablebuilder(
					    {fmt::format("{}", key.c_str())}, {&localData}, value_changed);
					if (value_changed)
					{
						nodebase->setDefaultValue(key, localData);
						setDataItemsWidth();
...
{
	ImGui::MenuItem("Lock", NULL, false,
	                false); /* no change DataMap in Free transformation */
    ...
    WorkspaceTransformation_s<ETransformType::Scale>::drawMenuSetDataMap()
{
	...
	{
		if (ImGui::MenuItem("Unlock", NULL, false,
		                    getLevelOfDetail() == WorkspaceLevelOfDetail::Full))
		{
	...
	{
		if (ImGui::MenuItem("Lock", NULL, false,
		                    getLevelOfDetail() == WorkspaceLevelOfDetail::Full))
		{
	...
	if (m_nodebase->as<Core::Transformation>()->hasSynergies())
	{
		if (ImGui::MenuItem("Disable synergies", NULL, false,
		                    getLevelOfDetail() != WorkspaceLevelOfDetail::Label))
		{
	        ...
	        ImGuiTableFlags_NoHostExtendX | ImGuiTableFlags_SizingFixedFit))
	{
		for (auto& [key, valueStore] : nodebase->getDefaultValues())
		{
			...
			case EValueType::Vec3:
			{
				...
				if (m_nodebase->as<Core::Transformation>()
				        ->hasSynergies()) /* uniform */
				{
					inner_interaction_happen |= drawDataSetValues_InsideTablebuilder(
					    {fmt::format("{}", key.c_str())}
					    /* \todo MH all/whole labels from core? */
					    ,
    ...
    WorkspaceTransformation_s<ETransformType::LookAt>::drawDataSetValues()
{
	...
	{
		index_of_change %=
		    columnLabels.size(); /* move to index of nodebase data column */
		nodebase->setDefaultValue(columnLabels[index_of_change],
		                          local_data[index_of_change]);
		setDataItemsWidth();
	}
