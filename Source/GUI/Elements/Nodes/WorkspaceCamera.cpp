...
WorkspaceCamera::WorkspaceCamera(ImTextureID headerBackground, WorkspaceCameraArgs const& args)
    : WorkspaceNodeWithCoreData(headerBackground, {.levelOfDetail=args.levelOfDetail, .headerLabel=args.headerLabel, .nodeLabel=args.nodeLabel, .nodebase=Core::Builder::createCamera()})
{
    ...
    m_sequence1 = std::make_shared<WorkspaceSequence>(headerBackground, {.nodebase=m_nodebase->as<Core::Camera>()->getProj()});
	m_sequence2 = std::make_shared<WorkspaceSequence>(headerBackground, {.nodebase=m_nodebase->as<Core::Camera>()->getView()});
...
WorkspaceCamera::WorkspaceCamera(ImTextureID headerBackground, std::string headerLabel, std::string nodeLabel)
    : WorkspaceNodeWithCoreData(headerBackground, Core::Builder::createCamera(), headerLabel, nodeLabel)
{
	fw.showMyPopup = false;
	fw.id = "";
	fw.value = NAN;
...
void WorkspaceCamera::drawDataFull(util::NodeBuilder& builder)
{
