...
void WorkspaceTransformation::updateSizes()
{
	...
	m_topOversizeSpace = std::max(
	    ...
	    m_topRectDiwne.GetWidth() -
	        std::max(
	            m_leftRectDiwne.GetWidth() + m_middleRectDiwne.GetWidth() + m_rightRectDiwne.GetWidth() +
	                ImGui::GetStyle().ItemSpacing.x * 2 /
	                    diwne
	                        .getWorkAreaZoom() /* space is between left-middle and middle-right, spacing is scaled in begin of frame */
	            ,
...
bool WorkspaceTransformation::beforeContent()
{
	...
	diwne.AddRectFilledDiwne(m_topRectDiwne.Min, m_bottomRectDiwne.Max,
	                         ImGui::ColorConvertFloat4ToU32(I3T::getTheme().get(EColor::NodeBgTransformation)), 5,
	                         ImDrawCornerFlags_Top); /* \todo JH 5 is rounding of corners -> take from Theme?*/
	return false;
}
...
bool WorkspaceTransformation::topContent()
{
	...
	diwne.AddRectFilledDiwne(m_topRectDiwne.Min, m_topRectDiwne.Max,
	                         ImGui::ColorConvertFloat4ToU32(I3T::getTheme().get(EColor::NodeHeaderTranformation)), 5,
	                         ImDrawCornerFlags_Top); /* \todo JH 5 is rounding of corners -> take from Theme?*/
...
bool WorkspaceTransformation::afterContent()
{
	    ...
	    dynamic_cast<WorkspaceDiwne&>(diwne).m_trackingFirstTransformation == this)
	{
		...
		if (m_inactiveMark != 0) /* \todo JH MH? kde prosím přečtu tracking? */
		{
			...
			if (dynamic_cast<WorkspaceDiwne&>(diwne).m_trackingFromLeft) { bottomright.x -= (1 - m_inactiveMark) * size.x; }
			else
			{
...
void WorkspaceTransformation::drawMenuSetDataMap()
{
	...
	bool enableSynergiesMenuItem = fullLOD &&
	    (m_nodebase->as<Core::Transformation>()
	         ->hasMenuSynergies()); // only some transformations have the possibility to set synergies
	...
	{
		if (ImGui::MenuItem("Lock", NULL, false, enableLockMenuItem)) { m_nodebase->as<Core::Transformation>()->lock(); }
	}
...
bool WorkspaceTransformation::drawDataSetValues_InsideTablebuilder(
    std::vector<std::string> const&
        labels /* labels have to be unique in node - otherwise change label passed to drawDragFloatWithMap_Inline() below */
    ,
    std::vector<float*> const& local_data, bool& value_changed)
{
	...
	Debug::Assert(number_of_values == local_data.size(),
	              "drawDataSetValues_InsideTablebuilder: All vectors (labels, local_data) must have same size.");
                                                             ...
                                                             int& index_of_change)
{
	...
	Debug::Assert(
	    number_of_values == local_data.size(),
	    "drawDataSetValuesTable_builder: columnLabels.size() * rowLabels.size() must be equal to local_data.size()");
