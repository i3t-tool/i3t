...
{
	if (m_renderTexture) delete (m_renderTexture);
...
{
	m_renderTexture =
	    new RenderTexture(&m_textureID, static_cast<int>(m_textureSize.x),
	...
	m_camera = new Camera(
	    60.0f, Application::get().world()->sceneRoot,
	    m_renderTexture); // version with the object shared with the 3D scene and positioned in the scene graph)
...
#define FLOOR_VEC2(_VAL) (ImVec2((float) (int) ((_VAL).x), (float) (int) ((_VAL).y))) // version of IM_FLOOR for Vec2
	...
	ImGui::Image(reinterpret_cast<ImTextureID>(m_textureID), zoomedTextureSize, ImVec2(0.0f, 1.0f),
	...
	interaction_happen |= diwne.IconButton(
	    ...
	    DIWNE::IconType::TriangleDownLeft, ImColor(100, 100, 150, 150), ImColor(100, 100, 150, 150), zoomedButtonSize,
	...
	interaction_happen |= diwne.IconButton(
	    DIWNE::IconType::TriangleDownRight, ImColor(100, 50, 50, 150), ImColor(100, 50, 50, 150),
	    DIWNE::IconType::TriangleDownRight, ImColor(100, 100, 150, 150), ImColor(100, 100, 150, 150), zoomedButtonSize,
	    ImVec4(buttonIconPadding, buttonIconPadding, buttonIconPadding, buttonIconPadding), true,
	...
	getNodebase()->setValue(m_textureSize.x / m_textureSize.y,
	                        1); // must be index 1, as there is a hidden output index 0, storing the incoming PV matrix
