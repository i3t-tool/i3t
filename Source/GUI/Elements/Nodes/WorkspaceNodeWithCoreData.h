...
struct WorkspaceNodeWithCoreDataArgs
{
	...
	Ptr<Core::NodeBase> nodebase =
			nullptr; /* \todo JH some default type of Core-Node will be nice (at least some of existing one) */
};
...
protected:
	...
	float m_dataItemsWidth =
			25.0f; /* \todo JH default number from some setting - just for safe if someone not call setDataItemsWidth() in construktor of child class... */
	float m_inactiveMark = 0.7f;
...
public:
	...
	WorkspaceLevelOfDetail		m_levelOfDetail =
			WorkspaceLevelOfDetail::Full; /* DG was not initialized - getLevelOfDetail returned garbage */
...
public:
	...
	virtual void drawDataSetValues_builder(util::NodeBuilder& builder, std::vector<std::string> const& labels,
	...
	virtual void drawInputPin(util::NodeBuilder& builder, Ptr<WorkspaceCorePinProperties> const& pinProp,
														Core::Pin* newLinkPin);
...
public:
	WorkspaceCorePinProperties(ne::PinId const id, std::string label, Core::Pin const& pin,
