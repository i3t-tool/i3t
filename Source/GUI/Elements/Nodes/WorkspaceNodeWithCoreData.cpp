...
WorkspaceNodeWithCoreData::WorkspaceNodeWithCoreData(
		...
		WorkspaceNodeWithCoreDataArgs const& args) /* \todo JH take default label from Const.h*/
		:
		WorkspaceNode(args.nodebase->getId(), headerBackground,
									{.headerLabel = args.headerLabel, .nodeLabel = args.nodeLabel}),
		m_nodebase(args.nodebase)
{
	...
	{
		m_workspaceInputsProperties.push_back(
				std::make_unique<WorkspaceCorePinProperties>(pin.getId()
																										 //, fmt::format("##{}", pin.getIndex())
																										 ,
																										 pin.getLabel(), pin, *this));

		m_workspaceLinksProperties.push_back(std::make_unique<WorkspaceLinkProperties>(pin.getId()));
	}

	for (Core::Pin const& pin : outputPins)
	{
		m_workspaceOutputsProperties.push_back(
				std::make_unique<WorkspaceCorePinProperties>(pin.getId()
																										 //, fmt::format("##{}", pin.getIndex())
																										 ,
																										 pin.getLabel(), pin, *this));
...
WorkspaceNodeWithCoreData::WorkspaceNodeWithCoreData(
		ImTextureID headerBackground, Ptr<Core::NodeBase> nodebase, std::string headerLabel,
		std::string nodeLabel) /* \todo JH take default label from Const.h*/
		:
		...
		m_nodebase(nodebase)
{
	...
	for (Core::Pin const& pin : inputPins)
	{
		...
		{
			m_workspaceInputsProperties.push_back(
					std::make_unique<WorkspaceCorePinProperties>(pin.getId(), fmt::format("##{}", pin.getIndex()), pin, *this));
		}
		else
		{
			m_workspaceInputsProperties.push_back(
					std::make_unique<WorkspaceCorePinProperties>(pin.getId(), pin.getLabel(), pin, *this));
		}
...
std::vector<Ptr<WorkspaceCorePinProperties>> const& WorkspaceNodeWithCoreData::getInputsProperties() const
{
	return m_workspaceInputsProperties;
}
std::vector<Ptr<WorkspaceCorePinProperties>> const& WorkspaceNodeWithCoreData::getOutputsProperties() const
{
	return m_workspaceOutputsProperties;
}
																														...
																														std::string const label)
{
	...
	bool valueChanged = ImGui::DragFloat(
			label.c_str(), value, step, 0.0f, 0.0f, fmt::format("%.{}f", getNumberOfVisibleDecimal()).c_str(),
			1.0f); /* \todo JH what parameter "power" mean? //SS if power >1.0f the number changes logaritmic */
...
void WorkspaceNodeWithCoreData::drawDataSetValues_builder(util::NodeBuilder&													builder,
																													std::vector<std::string> const&							labels,
																													std::vector<getter_function_pointer> const& getters,
																													std::vector<setter_function_pointer> const& setters,
																													std::vector<unsigned char>									datamap_values)
{
	...
	for (int i = 0; i < number_of_values; i++)
	{
		...
		if (drawDragFloatWithMap_Inline(&localData, datamap_values[i], fmt::format("##{}:ch{}", idOfNode, i))) //todo check
		{
			valueChanged		= true;
			index_of_change = i;
...
Core::Pin const& WorkspaceCorePinProperties::getCorePin() const { return m_pin; }

WorkspaceNodeWithCoreData& WorkspaceCorePinProperties::getNode() const { return m_node; }

int WorkspaceCorePinProperties::getIndex() const { return m_pin.getIndex(); }

ne::PinId const WorkspaceCorePinProperties::getParentPinId() const { return ne::PinId(m_pin.getParentPin()->getId()); }

PinKind WorkspaceCorePinProperties::getKind() const { return m_pin.isInput() ? PinKind::Input : PinKind::Output; }

EValueType WorkspaceCorePinProperties::getType() const { return m_pin.getType(); }
