...
bool saveWorkspace(FILE* f, std::vector<Ptr<WorkspaceNodeWithCoreData>>* _workspace, int at)
{
	...
	for (int i = 0; i < _workspace->size(); i++)
	{
		WorkspaceNodeWithCoreData* nodebasedata =
		    _workspace->at(i)
		        .get(); /* \todo JH this is confusing - in WorkspaceNodeWithCoreData are also graphic informations, data are in Ptr<Core::NodeBase> */
		...
		else if (strcmp(keyword, "Free") == 0)
		{
			...
			fprintf(f,
			        "int d%d=datamat4(%0.3ff,%0.3ff,%0.3ff,%0.3ff, %0.3ff,%0.3ff,%0.3ff,%0.3ff, %0.3ff,%0.3ff,%0.3ff,%0.3ff, "
			        "%0.3ff,%0.3ff,%0.3ff,%0.3ff);\n",
			        i + at, m[0][0], m[0][1], m[0][2], m[0][3], m[1][0], m[1][1], m[1][2], m[1][3], m[1][0], m[2][1], m[2][2],
		...
		else if (strcmp(keyword, "Ortho") == 0)
		{
			...
			fprintf(f,
			        "int d%d=datamat4(%0.3ff,%0.3ff,0.0f,0.0f, %0.3ff,%0.3ff,0.0f,0.0f, %0.3ff,%0.3ff,0.0f,0.0f, "
			        "%0.3ff,%0.3ff,0.0f,0.0f);\n",
			        i + at, m[0][0], m[0][1], m[1][0], m[1][1], m[2][0], m[2][1], m[3][0], m[3][1]);
	...
	for (int i = 0; i < _workspace->size(); i++)
	{
		...
		{
			fprintf(f, "confnode(n%d,%d,%s);\n", i + at, nodebasedata->getNumberOfVisibleDecimal(),
...
void Scripting::runCommand(std::string cmd)
{
	...
	if (strcmp(cmd.c_str(), "help") == 0) { scriptingHelp(0); }
...
void scriptingHelp(int page)
{
	...
	const char* pages[4];
	pages[0] = "int datamat4(float f1, float f2, ..., float f16)\n"
	...
	pages[1] = "int pulse(int x, int y, char* header)\n"
	           "int pulsec()\n"
