...
{
	if (s_self != nullptr) { delete s_self; }
}
                                   ...
                                   unsigned fromIndex, unsigned myIndex)
{
	...
	if (result != ENodePlugResult::Ok) return result;
	...
	for (auto& state : rightNode->m_OperatorState) state = EValueState::Locked;

	if (leftNode->getOutputPinsRef()[fromIndex].getType() != EValueType::Pulse) { leftNode->spreadSignal(fromIndex); }
...
std::vector<Ptr<NodeBase>> GraphManager::getAllInputNodes(const NodePtr& node)
{
	...
	for (size_t i = 0; i < inputsCount; ++i)
	{
		...
		if (parent != nullptr) result.push_back(parent);
	}
...
Ptr<NodeBase> GraphManager::getParent(const NodePtr& node, size_t index)
{
	...
	if (index > pins.size()) return nullptr;

	if (pins.empty() || pins[index].m_input == nullptr) { return nullptr; }

	auto expected = pins[index].m_input->getOwner();

	if (expected->m_owner != nullptr) { return expected->m_owner; }
	else
	{
		return expected;
	}
...
std::vector<Ptr<NodeBase>> GraphManager::getOutputNodes(const Ptr<Core::NodeBase>& node, size_t index)
{
	...
	for (const auto& other : othersInputs) result.push_back(other->getOwner());
...
{
	for (auto& cycle : s_self->m_cycles) cycle->update(tick);
}
...
SequenceTree::MatrixIterator SequenceTree::end()
{
	...
	Ptr<Sequence> parent;
	while ((parent = toSequence(GraphManager::getParent(cur, 0))) != nullptr) { cur = parent; }
...
void SequenceTree::MatrixIterator::withdraw()
{
	...
	if (m_currentMatrix == nullptr) index = -1;
	...
	if (m_currentMatrix == matrices.back())
	{
		...
		while ((prevsParent = toSequence(GraphManager::getParent(prev))) != m_currentSequence) { prev = prevsParent; }
...
void MatrixTracker::track()
{
	...
	if (Math::eq(0.0f, m_param)) { interpParam = 0.0f; }
