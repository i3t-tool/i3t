...
struct Operation
{
	...
	const bool isConstructor =
	    false; //< this is a standard operator node - processes only the connected pins and has no GUI  editable values
	...
	Operation(const std::string& keyWord, const std::string& defaultLabel, bool isConstructor, int numberOfInputs,
	          ...
	          const std::vector<EValueType>& outputTypes) :
	    keyWord(keyWord),
	    defaultLabel(defaultLabel), isConstructor(isConstructor), numberOfInputs(numberOfInputs), inputTypes(inputTypes),
	    numberOfOutputs(numberOfOutputs), outputTypes(outputTypes)
	{}
...
static const std::vector<Operation> operations = {
    ...
    {n(ENodeType::VecVecToQuat), "quatvec3, vec3", 2, twoVector3Input, 1, quatInput,
     "Input vector's are normalized."}, // quatvec3, vec3   //PF: floatInput changed to quatInput
    ...
    {n(ENodeType::MakeAxisAngle), "rotate", 2, floatVector3Input, 1, matrixInput, NO_TAG,
     AngleAxisInputNames},                                                                                 // rotate
    {n(ENodeType::MakeOrtho), "ortho", 6, sixFloatInput, 1, matrixInput, NO_TAG, orthoFrustrumInputNames}, // ortho
    {n(ENodeType::MakePerspective), "perspective", 4, fourFloatInput, 1, matrixInput, NO_TAG,
     PerspectiveInputNames}, // perspective
    {n(ENodeType::MakeFrustum), "frustum", 6, sixFloatInput, 1, matrixInput, NO_TAG,
...
FORCE_INLINE const Operation* getTransformProps(ETransformType type) { return &getTransformOperation(type).operation; }
...
FORCE_INLINE const std::map<std::string, EValueType>& getTransformDefaults(const std::string& keyWord)
{
	...
	if (type.has_value()) return getTransformOperation(*type).defaultValuesTypes;
	return noDefaults;
}
...
FORCE_INLINE const TransformMask& getTransformMap(const std::string& keyWord)
{
	...
	auto type = magic_enum::enum_cast<ETransformType>(keyWord);
	if (type.has_value()) return getTransformOperation(*type).mask;
	return noDefaults;
}
