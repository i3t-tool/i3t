...
void InputBindings::init()
{
	...
	InputManager::setInputAxis("rotateAround", 1.0f, Keys::mouseRight,
	                           {Keys::altl});
	...
	InputManager::setInputAction("TestMouseCtrlAction", Keys::mouseLeft,
	                             {Keys::ctrll}); /// \todo Will be removed
	InputManager::setInputAction(
	    "MyTestAction", Keys::t,
	    {Keys::ctrll, Keys::altl}); /// \todo Will be removed
	InputManager::setInputAxis("MyTestAxis", 1.0f, Keys::p,
	                           {Keys::ctrll}); /// \todo Will be removed
	setAxisKey("MyTestAxis", -1.0f, Keys::mouseRight,
	           {Keys::ctrll}); /// \todo Will be removed
#endif
	...
	InputManager::setInputAction(
	    "trackingSwitchOff",
	    Keys::down); /* \todo JH maybe some different key when more than 2
	                    tracking direction? */
	...
	InputManager::setInputAction("createAndPlugConstructor", Keys::mouseLeft,
	                             {Keys::ctrll});
...
const std::vector<InputBindings::ActionMapping>&
InputBindings::getActionMapping(const char* name)
{
...
std::vector<InputBindings::AxisMapping>
InputBindings::getAxisMapping(const char* name)
{
...
void InputBindings::setAxisKey(const char* name, float scale, Keys::Code code,
                               ModifiersList modifiers)
{
