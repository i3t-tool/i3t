...
glm::vec2 vecWorldscreen(glm::vec3 pos,
                         glm::vec3 dir); ///, Vector from world coordinates to screen coordinates (dimensions in pixels)
...
glm::vec3
mouseray(glm::vec2 pos); ///< Mouse cursor from screen coordinates to ray in world coordinates (z1=0.7, z2=0.9)
glm::mat4
          getOrtho(glm::mat4 transform,
                   int referenceAxis); ///< sets first three vectros to be orthonormal taken as vec3. Reference axis is kept.
...
glm::mat4 getFullTransform(
    GameObject* obj); ///< accumulate all model transformations from the object parents (up to parent == NULL)
...
glm::mat4 getProjParams(glm::mat4 projinv,
                        bool      isPersp); ///< get the parameters defining a projection transformation (near,far, ...)
void      setLen(glm::vec3* vec, float len); ///< set the length of the given vector vec
float     angle2(float x,
                 float y); ///< Slope angle of the line [0,0]-[x,y]. Oriented angle from the x-axis to this segment.
