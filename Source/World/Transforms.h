...
glm::vec2
world2screen(glm::vec3 pos); ///< Point from world coordinates to screen
                             ///< coordinates (dimensions in pixels)
glm::vec2
vecWorldscreen(glm::vec3 pos,
...
glm::vec2
mouse2viewport(glm::vec2 pos); ///< Mouse cursor from screen coordinates to
                               ///< viewport coordinates (<-1;1>^2)
glm::vec3
mouseray(glm::vec2 pos); ///< Mouse cursor from screen coordinates to ray in
...
glm::mat4 getNormalized(
    glm::mat4 transform); ///< normalize first three vectros taken as vec3.
...
glm::mat4 getFullTransform(
    GameObject* obj); ///< accumulate all model transformations from the object
                      ///< parents (up to parent == NULL)
glm::mat4
getNodeTransform(const Ptr<Core::NodeBase>* node,
                 const Ptr<Core::Sequence>* parent,
                 bool invLookAt = false); ///< get the PVMmatrix from core
glm::vec3 planeIntersect(
    glm::vec3 px, glm::vec3 py,
    glm::vec3 p0); ///< returns intersection point of nouse ray with given plane
glm::mat4
getProjParams(glm::mat4 projinv,
