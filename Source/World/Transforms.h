...
glm::mat4 getFullTransform(GameObject* obj);        ///< accumulate all model transformations from the object parents (up to parent == NULL) 
glm::mat4 getNodeTransform(const Ptr<Core::NodeBase>*node,const Ptr<Core::Sequence>*parent,bool invLookAt=false);///< get the PVMmatrix from core
glm::vec3 planeIntersect(glm::vec3 px,glm::vec3 py,glm::vec3 p0);///< returns intersection point of nouse ray with given plane
glm::mat4 getProjParams(glm::mat4 projinv,bool isPersp);///< get the parameters defining a projection transformation (near,far, ...)
void setLen(glm::vec3* vec, float len);   ///< set the length of the given vector vec
float angle2(float x, float y);  ///< Slope angle of the line [0,0]-[x,y]. Oriented angle from the x-axis to this segment.
