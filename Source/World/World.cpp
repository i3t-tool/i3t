...
World::World()
{
	     ...
	     i != this->manipulators.cend(); i++)
	{
		...
		{ //add each manipulator only once (rotation is stored multiple times under different keys)
...
void GUIRecursive(GameObject* root)
{
	...
	{
		if (root->components[i]->m_isActive) { root->components[i]->GUI(); }
	}
}
void updateRecursive(GameObject* root)
{
	...
	for (int i = 0; i < root->components.size(); i++)
	{
		if (root->components[i]->m_isActive) { root->components[i]->update(); }
	}
}
void startRecursive(GameObject* root)
{
	for (int i = 0; i < root->components.size(); i++)
	{
		if (root->components[i]->m_isActive) { root->components[i]->start(); }
	}
