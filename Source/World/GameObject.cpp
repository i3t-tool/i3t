...
void GameObject::draw(glm::mat4 parentTransform)
{
	...
	glUniformMatrix4fv(this->shader->PVMmatrix, 1, GL_FALSE,
...
GameObject::GameObject(const pgr::MeshData mesh, struct Shader* shader, GLuint texture)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * mesh.nVertices * mesh.nAttribsPerVertex, mesh.verticesInterleaved,
	             GL_STATIC_DRAW);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned) * mesh.nTriangles * 3, mesh.triangles,
	...
	glVertexAttribPointer(this->shader->attr_pos, 3, GL_FLOAT, GL_FALSE, mesh.nAttribsPerVertex * sizeof(float),
	                      (void*) (0)); // [xyz][nx,ny,nz][s,t]
	if (this->shader->attr_norm >= 0)
	{ // norm pos
		glEnableVertexAttribArray(this->shader->attr_norm);
		glVertexAttribPointer(this->shader->attr_norm, 3, GL_FLOAT, GL_FALSE, mesh.nAttribsPerVertex * sizeof(float),
		                      (void*) (4 * 3)); // [xyz][nx,ny,nz][s,t]
	}
	if (this->shader->attr_uv >= 0)
	{ // uv
		glEnableVertexAttribArray(this->shader->attr_uv);
		glVertexAttribPointer(this->shader->attr_uv, 2, GL_FLOAT, GL_FALSE, mesh.nAttribsPerVertex * sizeof(float),
...
void GameObject::setParent(GameObject* parent, bool keepTransform) { parent->addChild(this, keepTransform); }
bool GameObject::rmChild(GameObject* obj, bool keepTransform)
{
	for (int i = 0; i < this->children.size(); i++)
	{
		if (this->children[i] == obj)
		{
			...
			if (keepTransform)
			{ obj->transformation = GameObject::inheritedTransform(this) * this->transformation * obj->transformation; }
			return true;
		}
	}
	return false;
...
void GameObject::addChild(GameObject* obj, bool keepTransform)
{
	...
	{
		obj->transformation =
		    glm::inverse(GameObject::inheritedTransform(this) * this->transformation) * obj->transformation;
	}
}
