...
void Camera::renderRecursive(GameObject* obj, glm::mat4 parent, bool isTranspartentPass)
{
	...
	{
		if (obj->components[i]->m_isActive)
		{
			obj->components[i]->render(&parent, isTranspartentPass);
		}
	}
...
void Camera::update()
{
	...
	/*glDepthMask(GL_FALSE);//render overlaping transparent (replace "render
	transparent"), naive solution renderRecursive(m_sceneRoot,
	glm::mat4(1.0f),true);//render color, dont write depth - because of
	overlapping glDepthMask(GL_TRUE);
	glColorMask(GL_FALSE,GL_FALSE,GL_FALSE,GL_FALSE);
	renderRecursive(m_sceneRoot, glm::mat4(1.0f), true);//write depth, color is
	already rendered glColorMask(GL_TRUE,GL_TRUE, GL_TRUE, GL_TRUE);*/
	if (!this->m_mainCamera)
	{
