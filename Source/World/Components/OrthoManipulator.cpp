...
void OrthoManipulator::render(glm::mat4* parent, bool renderTransparent)
{
	...
	// glm::mat4
	// transform=glm::inverse(getNodeTransform(&m_editednode,&m_parent));//LookAt?
	...
	if (renderTransparent)
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(World::shaderProj.program,
		"P2Matrix"), 1, GL_FALSE, glm::value_ptr(projinv*m));
...
void OrthoManipulator::update()
{
	...
	// v=m_editednode->get()->setValue((glm::vec3)m_edited[3]);}
