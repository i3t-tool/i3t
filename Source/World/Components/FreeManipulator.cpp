...
void FreeManipulator::render(glm::mat4*parent,bool renderTransparent){
	...
	ManipulatorUtil::drawHandle(m_lineh,t,glm::vec4(1.0f,selected*1.5f,selected*1.5f,1.0f),m_stencilaxisx,false,m_hoverhandle);
	...
	ManipulatorUtil::drawHandle(m_lineh,t,glm::vec4(selected*1.5f,1.0f,selected*1.5f,1.0f),m_stencilaxisy,false,m_hoverhandle);
	...
	ManipulatorUtil::drawHandle(m_lineh,t,glm::vec4(selected*0.5f+0.1f,selected*0.5f+0.4f,1.0f,1.0f),m_stencilaxisz,false,m_hoverhandle);
	...
	if(m_editmode==FreeManipulator::EDIT_SCALE){//override scale handles for free edit
		...
		if(m_editaxis==0){
			...
			ManipulatorUtil::drawHandle(m_uniscaleh,m_handlespace,glm::vec4(1.0f,0.0f,0.0f,1.0f),m_stencilx,m_activehandle,m_hoverhandle);
		}
		else if(m_editaxis==1){
			m_uniscaleh->transformation=glm::mat4(1.0f)*scale;
			ManipulatorUtil::drawHandle(m_uniscaleh,m_handlespace,glm::vec4(0.0f,1.0f,0.0f,1.0f),m_stencily,m_activehandle,m_hoverhandle);
		}
		else if(m_editaxis==2){
			m_uniscaleh->transformation=glm::mat4(1.0f)*scale;
			ManipulatorUtil::drawHandle(m_uniscaleh,m_handlespace,glm::vec4(0.1f,0.4f,1.0f,1.0f),m_stencilz,m_activehandle,m_hoverhandle);
		}
		...
		ManipulatorUtil::drawHandle(handle,getOrtho(m_handlespace,0),glm::vec4(1.0f,0.0f,0.0f,1.0f),m_stencilx,m_activehandle,m_hoverhandle);
		handle->transformation=glm::rotate(glm::mat4(1.0f),glm::radians(-90.0f),glm::vec3(1.0f,0.0f,0.0f))*scale;
		ManipulatorUtil::drawHandle(handle,getOrtho(m_handlespace,1),glm::vec4(0.0f,1.0f,0.0f,1.0f),m_stencily,m_activehandle,m_hoverhandle);
		handle->transformation=glm::mat4(1.0f)*scale;
		ManipulatorUtil::drawHandle(handle,getOrtho(m_handlespace,2),glm::vec4(0.1f,0.4f,1.0f,1.0f),m_stencilz,m_activehandle,m_hoverhandle);	

		if (m_editmode == FreeManipulator::EDIT_POSITION){ // two-axis handles
...
void FreeManipulator::update(){ 
	...
	if(m_activehandle==-1){
		...
		else if(sel==stencilxyz){ m_hoverhandle =stencilxyz;}
	...
	if(InputManager::isKeyJustPressed(Keys::mouseLeft)){
		...
		else if(sel==stencilxyz){m_activehandle=stencilxyz;m_axisnum=3;m_axisnum2=-1;}
		else if(sel==m_stencilaxisx){m_editaxis=0;}
		else if(sel==m_stencilaxisy){m_editaxis=1;}
		else if(sel==m_stencilaxisz){m_editaxis=2;}
		else if(sel==m_stencilaxisw){m_editaxis=3;}
	...
	if(m_activehandle==-1){
		...
		else if (InputManager::isKeyPressed(Keys::z)){m_editaxis = 2;}
		else if (InputManager::isKeyPressed(Keys::w)){m_editaxis = 3;}
		m_bkp=m_edited;

		if(m_editaxis==3){
			glm::mat4 m= m_edited;m[2]=m[3];
	...
	//printf("hs %f %f %f\n\n", handlespace[3][0], handlespace[3][1], handlespace[3][2]);
	...
	if (m_editmode == FreeManipulator::EDIT_ROTATION) {
		if (transactionBegin) {
			...
			//glm::vec3 dir3 = glm::normalize(glm::cross(pc - (glm::vec3)ortho[3], (glm::vec3)axes[axisnum]));
			//dirbkp = glm::normalize(world2screen(p0) - world2screen(p0 - dir3));
		}
		dir=m_dirbkp;
	}
	...
	drag3*=1.2f;
	if(m_editmode==FreeManipulator::EDIT_ROTATION){
		glm::mat4 r=glm::mat4(m_rotfreebkp[0],m_rotfreebkp[1],m_rotfreebkp[2], m_rotfreebkp[2]);//axes of rotation (0,1,2,2)
