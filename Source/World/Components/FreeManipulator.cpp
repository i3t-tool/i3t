...
void FreeManipulator::update()
{
	...
	glm::vec2 spos2 = world2screen((glm::vec3)(
	    m_handlespace[3] +
	    m_handlespace * axes[m_axisnum])); //spos1,spos2 - project two points on screen - project axis on screen
	...
	if (m_axisnum2 != -1)
	{
		glm::vec2 spos22 = world2screen((glm::vec3)(
		    m_handlespace[3] + m_handlespace * axes[m_axisnum2])); //project two points on screen - project axis on screen
	...
	{
		glm::mat4 r =
		    glm::mat4(m_rotfreebkp[0], m_rotfreebkp[1], m_rotfreebkp[2], m_rotfreebkp[2]); //axes of rotation (0,1,2,2)
