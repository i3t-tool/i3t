...
struct Config
{
  ...
  ///< Information describing the definition - initial state of the scene description \todo Update to contain all key bindings from tabsConfig.cpp
  //@}
   ...
   * \param 		  	input		The std::string just read from the stream containing the name of the attribute which follows. Is provided by the file reader.
   * \param 		  	attribName 	The std::string with the attribute name to be read from the stream. This value is being compared with the input
   * \param [out]		val  		The read value is stored to the given variable.
   *
  ...
  */
  ...
  /**
   * Skip comments. This routine consumes either a single line comment (starting with # or //) or a multiline comment - Does not allow nested comments!
   * \todo the only method changing the value of input and only for multiline comments - change to blank...
   *
   * \param [in]	is   	The input stream.
  ...
  /**
      Read configuration of the application from the default configuration file (cfg_default.dcfg), or from the file passed as a command line parameter, if present.
    ...
    <DL>
      <DT>GUI_CFG file</DT>
       <DD>Definition of the properties of the GUI, such as colors of boxes and wires ( OpValueType), camera control (orbit or trackball), units (degs or radians), and sizes of boxes,... <BR>
           The GUI_CFG file (such as \\data\\gui.cfg) is read by this function using TabsConfig::readFromFile() and the GUI_CFG name is not stored in the configuration structure.</DD>
      <DT>CONTENT_FILE</DT>
       <DD>Enumeration of the \\data\\*.cfg files (textures, materials (Phong parameters + textures),  geometries (models), and objects (combines names, geometries and textures to groups of named objects)). <BR>
           This file (such as \\data\\content.cnt) is registered in main.init() and read by TabSpace::addContent</DD>
      <DT>LOAD_SCENE</DT>
        <DD>Initial scene loaded at program start.<BR>
          This file is read in main.init() by Reader::openSceneFile. Defalut value is \\data\\internal\\defScene.scn</DD>
    </DL>
