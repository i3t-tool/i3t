...
struct Config
{
	static std::string WORKING_DIRECTORY; ///< Pathname of the working directory -
	...
	/**
	    Read configuration of the application from the default configuration file
	  (cfg_default.dcfg), or from the file passed as a command line parameter, if
	  present.

	  Read values such as window size, background color, mouse sensitivity, camera
	  near and far plane. Reads also the other configuration files names, such as:
	  <DL>
	    <DT>GUI_CFG file</DT>
	     <DD>Definition of the properties of the GUI, such as colors of boxes and
	  wires ( OpValueType), camera control (orbit or trackball), units (degs or
	  radians), and sizes of boxes,... <BR> The GUI_CFG file (such as
	  \\data\\gui.cfg) is read by this function using TabsConfig::readFromFile()
	  and the GUI_CFG name is not stored in the configuration structure.</DD>
	  <DT>CONTENT_FILE</DT> <DD>Enumeration of the \\data\\*.cfg files (textures,
	  materials (Phong parameters + textures),  geometries (models), and objects
	  (combines names, geometries and textures to groups of named objects)). <BR>
	  This file (such as \\data\\content.cnt) is registered in main.init() and
	  read by TabSpace::addContent</DD> <DT>LOAD_SCENE</DT> <DD>Initial scene
	  loaded at program start.<BR> This file is read in main.init() by
	  Reader::openSceneFile. Defalut value is \\data\\internal\\defScene.scn</DD>
	  </DL>
	  It is called from the main.main() after command line parameters processing.
	  and directly loads the GUI_CFG file.
...
//#define I3T_RELEASE_STANDALONE
	 ...
	 * \param [out]		result 	The relative path with working directory stripped
	 * out.
	 *
	 ...
	 *
	 * \return	True if the path starts with a slash "/" indicating it is
	 * relative, false if not.
	 */
