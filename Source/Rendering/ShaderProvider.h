...
{
  /**
   * \brief Three groups of vectors of models: groups[0] to groups[3], each for all models using the same shader for display. 
  ...
  RenderGroups()
  {
    groups = std::vector<std::vector<ModelInstance *>>(SHADER_COUNT, std::vector<ModelInstance*>());
  }
};
...
public:
  ...
  * \return
  */
  static int init(const char* redVert, const char* redFrag,
  ...
  /**
   * \brief Add the model \a m to one of three groups according to the shader it will use for display. The selection is based on number of attributes and existence of the texture.
   * \param m Model to be added to the appropriate group
   */
  static void addToGroup(ModelInstance* m);
  /**
   * \brief Find the model in one of the groups and remove it from this group. Searches only one group with the appropriate number of vertex attributes.
   * \param m Model to be removed
   */
  ...
  /**
   * \brief Render all models in all groups using the projection and view matrices omitting one model given by \a noRender
   * \param proj			Projection matrix
   * \param view			View matrix
   * \param environment	Ambient and directional light.
   * \param noRender      Single model instance excluded from rendering (not rendered)
   */
  static void renderAll(glm::mat4 proj, glm::mat4 view, Environment* environment, ModelInstance* noRender);

  /**
   * \brief Connects the attributes for given geometry - Select the shader based on number of model attributes (8=>baseShader or other=>redShader)
   * \param g Geometry to be rendered. Appropriate shader method connectVertexAttributes() is called from shaders[]. 
   *          Implemented for red and base shaders, NOT implemented for AlphaShader
   */
