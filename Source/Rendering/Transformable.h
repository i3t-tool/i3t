...
public:
	 ...
	 */
	Transformable() { trans = glm::mat4(1.0f); }

	virtual ~Transformable() = default;
	//{};
	// matrix
	void setTrans(const glm::mat4 mat) { trans = mat; }

	/**
	 * \brief Get the stored transformation
	 * \return stored matrix
	 */
	virtual glm::mat4 getTrans() { return trans; }
	...
	// // set translation
	void setTranslation(const glm::vec3 vec) { trans[3] = glm::vec4(vec, 1.0f); }

	void setTranslation(const float x, const float y, const float z) { trans[3] = glm::vec4(x, y, z, 1.0f); }

	// // get position
	glm::vec3 getPosition() { return glm::vec3(trans[3]); }

	// rotation
	void rotate(const float rads, const glm::vec3 axis) { trans = glm::rotate(trans, rads, axis); }

	// // rotateX
	void rotateX(const float rads) { trans = glm::rotate(trans, rads, glm::vec3(1.0f, 0.0f, 0.0f)); }

	// // rotateY
	void rotateY(const float rads) { trans = glm::rotate(trans, rads, glm::vec3(0.0f, 1.0f, 0.0f)); }

	// // rotateZ
	void rotateZ(const float rads) { trans = glm::rotate(trans, rads, glm::vec3(0.0f, 0.0f, 1.0f)); }

	// scale
	void scale(const float s) { trans = glm::scale(trans, glm::vec3(s, s, s)); }

	void scale(const float sx, const float sy, const float sz) { trans = glm::scale(trans, glm::vec3(sx, sy, sz)); }

	float getScaleX() { return glm::length(glm::vec3(trans[0])); }

	float getScaleY() { return glm::length(glm::vec3(trans[1])); }

	float getScaleZ() { return glm::length(glm::vec3(trans[2])); }
