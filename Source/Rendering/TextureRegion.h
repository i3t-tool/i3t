...
protected:
  ...
  glm::vec4 uv; ///< uv coordinates of the sub-region of the glyph (single character BBOX) in the texture in the form vec4(umin, umax, vmin, vmax) \rename to BBOX?
...
public:
   ...
   *        Flips y, i.e. the input fontFile has the origin in the upper left corner, the OpenGL texture from the lower left corner.
   * \param _texId Square texture with all glyphs
   * \param texSize Size of the square texture with all the glyphs
   * \param x Absolute x-coordinate [texels] of the Top-left corner of the glyph region in the texture. x-coordinate is from range <0..texSize)
   * \param y Absolute y=coordinate [texels] of the Top left corner of the glyph region in the texture. y-coordinate is from range <0..texSize)
   * \param width  Width of the glyph rectangle in pixels
   * \param height Height of the glyph rectangle in pixels
   * 
  ...
  TextureRegion(const GLuint _texId, const int texSize, const float x, const float y, const float width,
                const float height) : glyphsTexId(_texId)
  {
   ...
   *        Flips y, i.e. the input fontFile has the origin in the upper left corner, the OpenGL texture from the lower left corner.
   * \param _texId
   * \param tu Relative u-coordinate of the Top left corner of the glyph region in the texture. u-coordinate from the range <0..1.0>
   * \param tv Relative v-coordinate of the Top left corner of the glyph region in the texture. v-coordinate from the range <0..1.0>
   * \param tWidth Width of the glyph rectangle in normalized uv coordinates <0..1.0>
   * \param tHeight Height of the glyph rectangle in normalized uv coordinates <0..1.0>
   */
  TextureRegion(const GLuint _texId, const float tu, const float tv, const float tWidth,
                const float tHeight) : glyphsTexId(_texId)
  {
   ...
   */
  void setTexId(const GLuint newGlyphsTexId)
  {
    glyphsTexId = newGlyphsTexId;
  }
  ...
  glm::vec2 getSize()
  {
    return glm::vec2(uv[1] - uv[0], uv[3] - uv[2]);
  }
  ...
  */
  glm::vec2 getCenter()
  {
    return glm::vec2((uv[1] + uv[0]) / 2.0f, (uv[3] + uv[2]) / 2.0f);
  }
...
public:
	...
	TextureRegionCycle(GLuint texId, float left, float down, float width, float height) : TextureRegion(texId, left, down, width, height) {
