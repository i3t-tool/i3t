...
private:
  ...
  void dispose()
  {
    if (generated)
    {
      ...
      if (depth) glDeleteRenderbuffers(1, &depthBufferID);
...
public:
   ...
   * @param buffer
   */
  static void setDefaultBuffer(unsigned int buffer)
  {
   ...
   */
  FrameBuffer(int _width, int _height, bool _depth) :
    generated(false), width(_width), height(_height), depth(_depth)
  {
    generate();
  }
