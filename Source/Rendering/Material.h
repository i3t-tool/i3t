...
public:
  ...
  Material(const std::string& name, const GLuint textureId, const glm::vec3 diffuseColor, const glm::vec3 specularColor,
           const float ns, const float ni, const float illum, const float d)
  {
    this->name = name;
    this->textureId = textureId;
...
public:
  ...
  // HC means Hard coded
  static Material* getHC(const std::string name)
  {
    return hcMaterials[name];
  }
  ...
  static void addHCMaterial(Material* mat)
  {
    hcMaterials[mat->getName()] = mat;
  }
