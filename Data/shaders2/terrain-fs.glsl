...
smooth in vec4 lposb;//light light space pos
smooth in vec4 lposbc;//light camera space pos
...
void main(){
	fragmentColor=texture(tex0,vec2(coord.x,-coord.y),-0.5)*0.5+texture(tex1,vec2(coord.x*0.04,-coord.y*0.04))*0.5;
	...
	float fog=pow(0.95,max(pos.y,0.0)*0.7);
